...
造船会社で働く従業員（外国人就業者を含む）を支える多言語Q&Aチャット。AIが版管理されたFAQ知識ベース（架空の「サンプル造船株式会社」の就業規則・安全衛生・作業基準・手続き）に基づき、利用者の言語で質問に答えます。

これは **Phase 1（チャットMVP）** の実装です。全体仕様は [`CLAUDE.md`](./CLAUDE.md) と [`docs/HANDOFF.md`](./docs/HANDOFF.md) を参照してください。

## 特徴

- **KBグラウンディング**: 回答は `data/faq.json`（26項目・就業規則/安全衛生/作業基準/手続き）に基づく。KBに無いことは断定せずスタッフ誘導。
...
data/faq.json         FAQ知識ベース（正本・26項目／就業規則・安全衛生・作業基準・手続き）
```
