...
日本（佐世保エリア）で働く外国人就業者（外国人材）を支える統合サポートアプリの **機能②「24時間・多言語Q&Aチャット」**。AIが版管理されたFAQ知識ベースに基づき、利用者の言語で生活・仕事・手続きの質問に答えます。

これは **Phase 1（チャットMVP）** の実装です。全体仕様は [`CLAUDE.md`](./CLAUDE.md) と [`docs/HANDOFF.md`](./docs/HANDOFF.md) を参照してください。

## 特徴

- **KBグラウンディング**: 回答は `data/faq.json`（32項目・生活/手続き/仕事/その他）に基づく。KBに無いことは断定せずスタッフ誘導。
...
data/faq.json         FAQ知識ベース（正本・32項目／生活・手続き・仕事・その他）
```
